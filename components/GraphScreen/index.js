...
const GraphScreen = ({ navigation }) => {
	const renderItem = ({ item }) => <Day hour={item.hour} timestamp={item.timestamp}/>;
	...
	return (
		...
		/>
			<FlatList data={current_cycle_values} renderItem={renderItem} keyExtractor={item => item.id} horizontal inverted />
		</SafeAreaView>
	);
};
